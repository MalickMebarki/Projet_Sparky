...
def spark_frame_generator(video_path, lower_colour, upper_colour, spark_threshold):
    ...
    while cap.isOpened():
        ...
        if cv.countNonZero(mask) > spark_threshold:
            yield frame, mask  # Générer la frame et le masque
...
def detect_lines_hough(frame):
    ...
    lines = cv.HoughLinesP(edges, 1, np.pi / 180, threshold=30, minLineLength=10, maxLineGap=200)

    if lines is not None and lines.size > 0:  # Vérifier que lines n'est pas vide
        return lines
    return []

def filter_lines_by_angle(lines, min_angle=-45, max_angle=45):
    """
    ...
    for line in lines:
        x1, y1, x2, y2 = line[0]
        angle = np.arctan2(y2 - y1, x2 - x1) * 180 / np.pi  # Calcul de l'angle en degrés
        if angle < min_angle or angle > max_angle:  # Garder les lignes dont l'angle est en dehors de [-25°, 25°]
            filtered_lines.append(line)
            angles.append(angle)  # Ajouter l'angle à la liste si la ligne est conservée
...
def draw_lines(frame, lines):
    ...
    for line in lines:
        x1, y1, x2, y2 = line[0]
        cv.line(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)  # Dessiner en vert
    return frame

def save_spark_frames_with_filtered_hough(video_path, lower_colour, upper_colour, spark_threshold, output_folder):
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    frame_count = 0
    all_angles = []  # Liste pour stocker tous les angles des lignes
    ...
    for frame, mask in spark_frame_generator(video_path, lower_colour, upper_colour, spark_threshold):
        ...
        if lines is not None and lines.size > 0:  # Si des lignes ont été détectées
...
def plot_angle_distribution(angles):
    ...
    plt.hist(angles, bins=50, range=(-90, 90), color='blue', edgecolor='black')
    plt.title("Distribution Angulaire des Lignes (|Angle| > 25°)")
...
lower_colour = np.array([130, 30, 30])  # Limite inférieure pour les étincelles roses
upper_colour = np.array([180, 255, 255])  # Limite supérieure pour les étincelles roses
spark_threshold = 8500  # Nombre minimum de pixels pour détecter une étincelle
...
output_folder = "frames_with_filtered_hough_lines"

# Sauvegarder les frames avec les lignes filtrées et afficher la distribution des angles
save_spark_frames_with_filtered_hough(video_path, lower_colour, upper_colour, spark_threshold, output_folder)
