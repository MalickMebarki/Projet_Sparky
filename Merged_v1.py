...
#SECTION 1 (générateur de frames)
...
def lin_reg_angle_mean(angles):
    ...
    while i < len(angles):
        ...
        i += 1
    mean = m/i
    return print(f"The angle mean using lin reg is {mean}")
...
def hough_angle_mean(angles):
    ...
    while i < len(angles):
        ...
        i += 1
    mean = m/i
    return print(f"The angle mean using hough is {mean}")
...
def main(video_path, lower_colour, upper_colour, spark_threshold, output_folder):
    ...
    for frame, mask in spark_frame_generator(video_path, lower_colour, upper_colour, spark_threshold):
        ...
        if lines is not None and len(lines) > 0:
            ...
            filtered_line_filename = os.path.join(filtered_lines_folder, f"filtered_lines_frame_{frame_count}.jpg")
        ...
        if line_params is not None:
            ...
            regression_line_filename = os.path.join(regression_lines_folder, f"regression_lines_frame_{frame_count_reg}.jpg")
    ...
    hough_angle_mean(all_angles_hough)
    lin_reg_angle_mean(all_angles_reg)
