...
def cos2_pdf(x: float):
    """
    Fonction de densité de probabilité
    """
    # Centre la courbe en 90°
    x_rad = np.radians(x - 90)
    return np.cos(x_rad)**2

def cos2_cdf(x: float, lower_bound=0, upper_bound=180):
    """
    Fonction de répartition
    """
    # Calcul de l'intégrale de la fonction de probabilité de densité
    result, _ = quad(cos2_pdf, lower_bound, x)
    # Normalisation avec l'intégrale totale
    normalization, _ = quad(cos2_pdf, lower_bound, upper_bound)
    return result / normalization

def cos2_quantile(p: float, lower_bound=0, upper_bound=180):
    """
    Trouver les racines de CDF(x)=p
    """
    # Recherche des racines de CDF(x)=p
    func = lambda x: cos2_cdf(x, lower_bound, upper_bound) - p

    # Utilisation de bisect pour s'assurer que la valeur est dans le bon intervalle
    return bisect(func, lower_bound, upper_bound)

def generate_theoretical_quantiles(n: int, lower_bound=0, upper_bound=180):
    """
    Génère les quantiles théoriques pour n données
    """
    probabilities = [(i + 1) / (n + 1) for i in range(n)]
    quantiles = [cos2_quantile(p, lower_bound, upper_bound) for p in probabilities]
    return quantiles

def qqplot_cos2_reg(observed_data: list[float], lower_bound=0, upper_bound=180):
    """
    Génère le qqplot de la distribution cos carré
    """
    n = len(observed_data)
    ...
    plt.figure(figsize=(8, 6))
    plt.scatter(observed_sorted, theoretical_quantiles, color='blue')
    plt.plot(observed_sorted, observed_sorted, color='red', linestyle='--')
    plt.xlabel('Quantiles observés')
    plt.ylabel('Quantiles théoriques')
    plt.title('Q-Q Plot pour une distribution cos^2 avec régression linéaire (Centré à 90°)')
    plt.show()
...
def qqplot_cos2_hough(observed_data: list[float], lower_bound=0, upper_bound=180):
    """
    Génère le qqplot de la distribution cos carré
    """
    n = len(observed_data)
    ...
    plt.xlabel('Quantiles observés')
    plt.ylabel('Quantiles théoriques')
    plt.title('Q-Q Plot pour une distribution cos^2 avec Hough (Centré à 90°)')
    plt.show()
...
def main(video_path, lower_colour: list[int], upper_colour: list[int], spark_threshold: int, output_folder):
    ...
    # Trace les qqplots de la distribution cos carré
    qqplot_cos2_reg(all_angles_reg)
    qqplot_cos2_hough(all_angles_hough)
