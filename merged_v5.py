...
#SECTION 2 (générateur de frames + fonction mean)
...
#SECTION 3 (linear regression with clustering)
...
def mean_reg(angles):
    ...
    while i < len(angles):
        k += angles[i]
        i += 1
    print(f"L'angle moyen avec la reg lin est {k/i}")
    return k/i
...
def compute_hough_space(edges): #ATTENTION C'EST L'ANGLE PAR RAPPORT A LA VERTICALE JE DOIS ENCORE CHANGER
    ...
    thetas = np.linspace(-np.pi/2, np.pi/2, 180)  # Theta from 0 to pi (angle between the horizontal axis and rho)
...
def mean_hough(angles):
    ...
    while i < len(angles):
        k += angles[i]
        i += 1
    print(f"L'angle moyen avec hough est {k/i}")
    return k/i
...
def sd_hough(angles):
    ...
    while i < len(angles):
        ...
        i += 1
    h = k/i
    print(f"L'écart type avec hough est {h**(1/2)}")
    return h**(1/2)

def qqhough(angles, moy, et):
    angles_array = np.array(angles)
    sm.qqplot(angles_array, line='45', loc = moy , scale = et)  # '45' adds a reference 45-degree line
    plt.title("QQ Plot hough")
    plt.show()
