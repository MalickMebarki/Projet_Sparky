...
def detect_lines_hough(frame):
    ...
    # Extract the 'V' channel (value, brightness)
    value_channel = hsv[:, :, 2]

    # Use Canny edge detection on the value channel
    edges = cv.Canny(value_channel, 50, 150, apertureSize=3)

    # Apply the Hough Transform to detect lines in the edges image
...
def filter_lines_by_angle(lines, min_angle=-5, max_angle=5):
    """
    ...
    for line in lines:
        ...
        if angle < min_angle or angle > max_angle:  # Garder les lignes dont l'angle est en dehors de [-25°, 25°]
            ...
            angles.append(abs(angle))  # Ajouter l'angle à la liste si la ligne est conservée
